...
- **사용한 이유 (성능 최적화):**
  ...
  마우스 스크롤 이벤트(`window.scrollY`)를 사용해 요소의 위치를 직접 계산하는 방식은 스크롤할 때마다 함수가 수백 번씩 실행되어 화면이 버벅이는 등 성능에 큰 부담이 됩니다. 이를 해결하기 위해, 브라우저가 백그라운드에서 자체적으로 요소의 화면 진입 여부를 감지해주는 최적화된 API인 `IntersectionObserver`를 사용하여 성능을 대폭 개선했습니다.

- **작동 방식:**

  1. **관찰 등록:** 먼저 `.fade-in` 클래스를 가진 숨겨진 요소들을 관찰 대상으로 등록합니다.
  2. **조건 감지 (20% 진입):** 사용자가 화면을 자연스럽게 내리다가, 관찰 중인 특정 요소가 화면(뷰포트) 안으로 **20% 이상 들어와 모습을 드러내는 순간**을 감지합니다. (즉, 스크롤 시작 조건이 아니라 '애니메이션 발동 조건'입니다.)
  3. **애니메이션 트리거:** 감지되는 즉시 해당 요소에 `.appear` 클래스를 추가하여, CSS에 정의된 `opacity`와 `transform` 전환 애니메이션을 실행해 시각적으로 스르륵 나타나게 만듭니다.
  4. **관찰 중단 (최적화):** 한 번 등장하여 애니메이션이 완료된 요소는 `unobserve`를 통해 관찰을 중단하여 불필요한 성능 낭비를 완벽히 차단합니다.
